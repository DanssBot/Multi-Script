...
pid_inst(){
  ...
  for list in $proto; do
    case $list in
      dropbear|python|stunnel4|v2ray|badvpn|squid|"$v_node") portas2=$(echo "$portas"|grep -w "LISTEN"|grep -w "$list") && [[ $(echo "${portas2}"|grep "$list") ]] && inst[$list]="\033[1;32m[ON] " || inst[$list]="\033[1;31m[OFF]";;
  ...
  if [[ $(systemctl is-active chekuser) = "active" ]]; then
    if [[ $(systemctl show --property MainPID --value chekuser) = 0 ]]; then
      inst[chksr]="\033[1;31m[OFF]"
    else
      inst[chksr]="\033[1;32m[ON]"
    fi
  else
    inst[chksr]="\033[1;31m[OFF]"
  fi
...
echo -ne "$(msg -verd "  [4]")$(msg -verm2 "➛") $(msg -azu "V2RAY         ${inst[v2ray]}")" && echo -e "$(msg -verd " [10]")$(msg -verm2 "➛") $(msg -azu "WIREGUARD     ${inst[wg]}")" #&& echo -e "$(msg -verd " [10]")$(msg -verm2 ">") $(msg -azu "SHADOW-NORMAL $(pid_inst ssserver)")"
echo -ne "$(msg -verd "  [5]")$(msg -verm2 "➛") $(msg -azu "OVER WEBSOCKET${inst[$v_node]}")" && echo -e "$(msg -verd " [11]")$(msg -verm2 "➛") $(msg -azu "CHECKUSER DATE${inst[chksr]}")"
...
echo -ne "$(msg -verd " [15]")$(msg -verm2 "➛") $(msg -azu "ADMIN PORTS ACTIVOS")" && echo -e "$(msg -verd " [20]")$(msg -verm2 "➛") $(msg -azu "INTERFACES MENUES")"
echo "$(msg -verd " [16]")$(msg -verm2 "➛") $(msg -azu "GEN DOMI/CERT-SSL") $([[ -z $(ls "${ADM_crt}") ]] && echo -e "\033[1;31m○ " || echo -e "\033[1;32m◉ ")"
msg -bar
echo -e "$(msg -verd " [21]") $(msg -verm2 "➛") $(msg -blu "ADMINISTRADOR DE ARCHIVOS WEB")"
echo -e "$(msg -verd " [22]") $(msg -verm2 "➛") $(msg -teal "HERRAMIENTAS y EXTRAS")"
msg -bar
